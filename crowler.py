...
developer_keys = itertools.cycle(DEVELOPER_KEYS)
...
def switch_key():
    ...
    try:
        new_key = next(developer_keys)
        print(f"Chave de API trocada para: {new_key}")
        log_information(f"Chave de API trocada para: {new_key}")
        request_count = 0
        ...
        log_information("Iniciando estado de hibernação de 24 horas e 15 minutos")
        time_sleep = (24 * 60 * 60) + (15 * 60)
        time.sleep(time_sleep)
        print("Finalizando estado de hibernação")
        log_information("Finalizando estado de hibernação")
        developer_keys = itertools.cycle(DEVELOPER_KEYS)
        new_key = next(developer_keys)
        return build(YOUTUBE_API_SERVICE_NAME, YOUTUBE_API_VERSION, developerKey=new_key)
